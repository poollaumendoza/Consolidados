    ...
    public class ContratoPrecinto
    {
        ...
        public List<EntityLayer.ContratoPrecinto> Listar(int IdContrato)
        {
            ...
            try
            {
                ...
                {
                    string query =
                        "Select cp.IdContratoPrecinto, cp.IdContrato, c.NroContratoLote, cp.IdContratoContenedor, cc.NroContenedor, cp.NroPrecinto, cp.IdEstado, e.NombreEstado from ContratoPrecinto cp join Contrato c on cp.IdContrato = c.IdContrato join ContratoContenedor cc on cp.IdContratoContenedor = cc.IdContratoContenedor join Estado e on cp.IdEstado = e.IdEstado where cp.IdContrato = @IdContrato";
                    SqlCommand Cmd = new SqlCommand(query, Cnx);
        ...
        public List<EntityLayer.ContratoPrecinto> Listar(string objeto, object valor)
        {
            ...
            switch (objeto)
            {
                ...
                case "IdContratoPrecinto":
                    query =
                        "Select cp.IdContratoPrecinto, cp.IdContrato, c.NroContratoLote, cp.IdContratoContenedor, cc.NroContenedor, cp.NroPrecinto, cp.IdEstado, e.NombreEstado from ContratoPrecinto cp join Contrato c on cp.IdContrato = c.IdContrato join ContratoContenedor cc on cp.IdContratoContenedor = cc.IdContratoContenedor join Estado e on cp.IdEstado = e.IdEstado where cp.IdContratoPrecinto = @IdContratoPrecinto";
                    break;
            }
